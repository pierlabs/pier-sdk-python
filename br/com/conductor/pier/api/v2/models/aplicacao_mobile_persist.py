# coding: utf-8

"""
Copyright 2016 SmartBear Software

    Licensed under the Apache License, Version 2.0 (the "License");
    you may not use this file except in compliance with the License.
    You may obtain a copy of the License at

        http://www.apache.org/licenses/LICENSE-2.0

    Unless required by applicable law or agreed to in writing, software
    distributed under the License is distributed on an "AS IS" BASIS,
    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
    See the License for the specific language governing permissions and
    limitations under the License.

    Ref: https://github.com/swagger-api/swagger-codegen
"""

from pprint import pformat
from six import iteritems


class AplicacaoMobilePersist(object):
    """
    NOTE: This class is auto generated by the swagger code generator program.
    Do not edit the class manually.
    """
    def __init__(self):
        """
        AplicacaoMobilePersist - a model defined in Swagger

        :param dict swaggerTypes: The key is attribute name
                                  and the value is attribute type.
        :param dict attributeMap: The key is attribute name
                                  and the value is json key in definition.
        """
        self.swagger_types = {
            'id_plataforma_mobile': 'int',
            'id_emissor': 'int',
            'token': 'str',
            'cor': 'str',
            'som': 'str',
            'icone': 'str',
            'certificado_base64': 'str',
            'senha': 'str'
        }

        self.attribute_map = {
            'id_plataforma_mobile': 'idPlataformaMobile',
            'id_emissor': 'idEmissor',
            'token': 'token',
            'cor': 'cor',
            'som': 'som',
            'icone': 'icone',
            'certificado_base64': 'certificadoBase64',
            'senha': 'senha'
        }

        self._id_plataforma_mobile = None
        self._id_emissor = None
        self._token = None
        self._cor = None
        self._som = None
        self._icone = None
        self._certificado_base64 = None
        self._senha = None

    @property
    def id_plataforma_mobile(self):
        """
        Gets the id_plataforma_mobile of this AplicacaoMobilePersist.
        Apresenta o identificador da plataforma.

        :return: The id_plataforma_mobile of this AplicacaoMobilePersist.
        :rtype: int
        """
        return self._id_plataforma_mobile

    @id_plataforma_mobile.setter
    def id_plataforma_mobile(self, id_plataforma_mobile):
        """
        Sets the id_plataforma_mobile of this AplicacaoMobilePersist.
        Apresenta o identificador da plataforma.

        :param id_plataforma_mobile: The id_plataforma_mobile of this AplicacaoMobilePersist.
        :type: int
        """
        self._id_plataforma_mobile = id_plataforma_mobile

    @property
    def id_emissor(self):
        """
        Gets the id_emissor of this AplicacaoMobilePersist.
        Apresenta o identificador do emissor.

        :return: The id_emissor of this AplicacaoMobilePersist.
        :rtype: int
        """
        return self._id_emissor

    @id_emissor.setter
    def id_emissor(self, id_emissor):
        """
        Sets the id_emissor of this AplicacaoMobilePersist.
        Apresenta o identificador do emissor.

        :param id_emissor: The id_emissor of this AplicacaoMobilePersist.
        :type: int
        """
        self._id_emissor = id_emissor

    @property
    def token(self):
        """
        Gets the token of this AplicacaoMobilePersist.
        Apresenta o token da aplicacao.

        :return: The token of this AplicacaoMobilePersist.
        :rtype: str
        """
        return self._token

    @token.setter
    def token(self, token):
        """
        Sets the token of this AplicacaoMobilePersist.
        Apresenta o token da aplicacao.

        :param token: The token of this AplicacaoMobilePersist.
        :type: str
        """
        self._token = token

    @property
    def cor(self):
        """
        Gets the cor of this AplicacaoMobilePersist.
        Apresenta a cor da mensagem da aplicacao.

        :return: The cor of this AplicacaoMobilePersist.
        :rtype: str
        """
        return self._cor

    @cor.setter
    def cor(self, cor):
        """
        Sets the cor of this AplicacaoMobilePersist.
        Apresenta a cor da mensagem da aplicacao.

        :param cor: The cor of this AplicacaoMobilePersist.
        :type: str
        """
        self._cor = cor

    @property
    def som(self):
        """
        Gets the som of this AplicacaoMobilePersist.
        Apresenta o som da mensagem da aplicacao

        :return: The som of this AplicacaoMobilePersist.
        :rtype: str
        """
        return self._som

    @som.setter
    def som(self, som):
        """
        Sets the som of this AplicacaoMobilePersist.
        Apresenta o som da mensagem da aplicacao

        :param som: The som of this AplicacaoMobilePersist.
        :type: str
        """
        self._som = som

    @property
    def icone(self):
        """
        Gets the icone of this AplicacaoMobilePersist.
        Apresenta o icone da mensagem da aplica\u00C3\u00A7\u00C3\u00A3o.

        :return: The icone of this AplicacaoMobilePersist.
        :rtype: str
        """
        return self._icone

    @icone.setter
    def icone(self, icone):
        """
        Sets the icone of this AplicacaoMobilePersist.
        Apresenta o icone da mensagem da aplica\u00C3\u00A7\u00C3\u00A3o.

        :param icone: The icone of this AplicacaoMobilePersist.
        :type: str
        """
        self._icone = icone

    @property
    def certificado_base64(self):
        """
        Gets the certificado_base64 of this AplicacaoMobilePersist.
        Apresenta o certificado da aplica\u00C3\u00A7\u00C3\u00A3o.

        :return: The certificado_base64 of this AplicacaoMobilePersist.
        :rtype: str
        """
        return self._certificado_base64

    @certificado_base64.setter
    def certificado_base64(self, certificado_base64):
        """
        Sets the certificado_base64 of this AplicacaoMobilePersist.
        Apresenta o certificado da aplica\u00C3\u00A7\u00C3\u00A3o.

        :param certificado_base64: The certificado_base64 of this AplicacaoMobilePersist.
        :type: str
        """
        self._certificado_base64 = certificado_base64

    @property
    def senha(self):
        """
        Gets the senha of this AplicacaoMobilePersist.
        Apresenta a senha da aplica\u00C3\u00A7\u00C3\u00A3o.

        :return: The senha of this AplicacaoMobilePersist.
        :rtype: str
        """
        return self._senha

    @senha.setter
    def senha(self, senha):
        """
        Sets the senha of this AplicacaoMobilePersist.
        Apresenta a senha da aplica\u00C3\u00A7\u00C3\u00A3o.

        :param senha: The senha of this AplicacaoMobilePersist.
        :type: str
        """
        self._senha = senha

    def to_dict(self):
        """
        Returns the model properties as a dict
        """
        result = {}

        for attr, _ in iteritems(self.swagger_types):
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                result[attr] = value

        return result

    def to_str(self):
        """
        Returns the string representation of the model
        """
        return pformat(self.to_dict())

    def __repr__(self):
        """
        For `print` and `pprint`
        """
        return self.to_str()

    def __eq__(self, other):
        """
        Returns true if both objects are equal
        """
        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """
        Returns true if both objects are not equal
        """
        return not self == other

