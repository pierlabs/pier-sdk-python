...
class WebhookApi(object):
    ...
    def salvar_using_post29(self, tipo_evento, url, **kwargs):
        """
        Salvar Webhook
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.salvar_using_post29(tipo_evento, url, callback=callback_function)
        ...
        # verify the required parameter 'tipo_evento' is set
        if ('tipo_evento' not in params) or (params['tipo_evento'] is None):
            raise ValueError("Missing the required parameter `tipo_evento` when calling `salvar_using_post29`")
        # verify the required parameter 'url' is set
        if ('url' not in params) or (params['url'] is None):
            raise ValueError("Missing the required parameter `url` when calling `salvar_using_post29`")
